...
public class InventoryView extends View implements IInventoryView
{
	...
	private void createContextInfoPanel()
	{
		...
		Font font =
				createFont(_contextInfoUnitLabel.getFont(), ContextInfoFontSize);
	...
	private void createItemTable()
	{
		...
		_itemTableScrollPane =
				new JScrollPane(_itemTable,
	...
	private void createProductContainerTree()
	{
		...
		_productContainerTreeScrollPane =
				new JScrollPane(_productContainerTree,
						ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
						ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		_productContainerTreeScrollPane
		...
		ActionListener actionListener =
				createProductContainerTreeActionListener();
		...
		return new MouseAdapter()
		{
			...
			private void handleMouseEvent(MouseEvent e)
			{
				...
				ProductContainerTreeNode node =
						(ProductContainerTreeNode) TreeOperations
	...
	private void createProductTable()
	{
		...
		_productTable =
				new JTable(_productTableModel, _productTableColumnModel);
		...
		_productTableScrollPane =
				new JScrollPane(_productTable,
						ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
						ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	...
	public void deleteProductContainer(ProductContainerData deletedContainer)
	{
		...
		try
		{
			ProductContainerTreeNode deletedNode =
					_productContainers.get(deletedContainer);
	...
	public void displayAddItemBatchView()
	{
		...
		DialogBox dialogBox =
				new DialogBox(_parent, "Add Items to " + target.getName());
		AddItemBatchView dialogView =
				new AddItemBatchView(_parent, dialogBox, target);
	...
	public void displayAddProductGroupView()
	{
		...
		AddProductGroupView dialogView =
				new AddProductGroupView(_parent, dialogBox, container);
		dialogBox.display(dialogView, false);
	}
	...
	public void displayEditProductGroupView()
	{
		...
		EditProductGroupView dialogView =
				new EditProductGroupView(_parent, dialogBox, target);
		dialogBox.display(dialogView, false);
	}
	...
	public void displayEditProductView()
	{
		...
		EditProductView dialogView =
				new EditProductView(_parent, dialogBox, target);
		dialogBox.display(dialogView, false);
	}
	...
	public void displayEditStorageUnitView()
	{
		...
		EditStorageUnitView dialogView =
				new EditStorageUnitView(_parent, dialogBox, target);
		dialogBox.display(dialogView, false);
	}
	...
	public void displayRemoveItemBatchView()
	{
		...
		RemoveItemBatchView dialogView =
				new RemoveItemBatchView(_parent, dialogBox);
		dialogBox.display(dialogView, false);
	}
	...
	public void displayTransferItemBatchView()
	{
		ProductContainerData target = getSelectedProductContainer();
		DialogBox dialogBox =
				new DialogBox(_parent, "Transfer Items to " + target.getName());
		TransferItemBatchView dialogView =
				new TransferItemBatchView(_parent, dialogBox, target);
		dialogBox.display(dialogView, false);
	}
	...
	public ProductData getSelectedProduct()
	{
		...
		if(selectedIndex >= 0)
		{
			ProductFormatter formatter =
					(ProductFormatter) _productTableModel.getValueAt(
							selectedIndex, 0);
	...
	{
		ProductContainerTreeNode selectedNode =
				getSelectedProductContainerNode();
			...
			ProductContainerData newContainer, int index)
	{
		...
		{
			ProductContainerTreeNode parentNode =
					_productContainers.get(parent);
			assert parentNode != null;
			if(parentNode != null)
			{
				parent.insertChild(newContainer, index);

				ProductContainerTreeNode newNode =
						new ProductContainerTreeNode(newContainer);
	...
	protected void layoutComponents()
	{
		...
		_rightSplitPane =
				new JSplitPane(JSplitPane.VERTICAL_SPLIT, _productPanel,
						_itemTableScrollPane);
			...
			String newName, int newIndex)
	{
		...
		try
		{
			ProductContainerTreeNode renamedNode =
					_productContainers.get(renamedContainer);
	...
	public void selectProduct(ProductData product)
	{
		...
		try
		{
			...
			{
				ProductFormatter formatter =
						(ProductFormatter) _productTableModel.getValueAt(i, 0);
	...
	public void setProductContainers(ProductContainerData rootData)
	{
		...
		{
			_productContainers =
					new HashMap<ProductContainerData, ProductContainerTreeNode>();
...
class ProductContainerTreeCellRenderer extends DefaultTreeCellRenderer
{
	...
	{
		_storageUnitIcon =
				new ImageIcon("images" + java.io.File.separator
						+ "door-icon.png");
	}
